...
public class ContestRepository implements IContestRepository {
    ...
    //Self -> from previous points
    @Override
    public List<Contest> findAllContestLevelWise(Level level) 
    {
        List<Contest> contest_list = new ArrayList<>();
      for(Map.Entry<String,Contest> i: contestMap.entrySet())
      {
        if(i.getValue().getLevel() == level)
        {
            contest_list.add(i.getValue());
        }
      }
      return contest_list;
    }
